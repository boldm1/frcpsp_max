...
def mip_solve(project):    
    ...
    model.addConstrs((q[r,j,t] >= project.tasks[j].alpha[r]*q[0,j,t] + project.tasks[j].beta[r]*(x[j,t] - y[j,t]) for j in N for r in project.tasks[j].r_dep for t in T_act[j]), name = "(44)")
    model.addConstrs((q[0,j,t] >= project.tasks[j].w*(v[j,t+1] - v[j,t]) for j in N for t in T_act[j]), name = "(45)")
    ...
    model.addConstrs((x[SS[0],t+SS[1]] <= x[j,t] for j in N for SS in project.tasks[j].raw_successors[0] if SS[0] != n+1 for t in ST_act[j] if t+SS[1] in ST_act[SS[0]]), name = "(34a)")
    model.addConstrs((y[SF[0],t+SF[1]] <= x[j,t] for j in N for SF in project.tasks[j].raw_successors[1] if SF[0] != n+1 for t in ST_act[j] if t+SF[1] in FT_act[SF[0]]), name = "(34b)")
    model.addConstrs((x[FS[0],t+FS[1]] <= y[j,t] for j in N for FS in project.tasks[j].raw_successors[2] if FS[0] != n+1 for t in FT_act[j] if t+FS[1] in ST_act[FS[0]]), name = "(34c)")
    model.addConstrs((y[FF[0],t+FF[1]] <= y[j,t] for j in N for FF in project.tasks[j].raw_successors[3] if FF[0] != n+1 for t in FT_act[j] if t+FF[1] in FT_act[FF[0]]), name = "(34d)")
